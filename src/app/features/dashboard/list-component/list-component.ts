...
@Component({
    ...
    template: `
        <div class="bg-base-300 h-fit space-y-4 rounded-xl p-4 shadow-md">
            <h2
                class="text-secondary flex items-center gap-2 text-lg font-bold"
            >
                {{ list().title }}
                <button
            ...
            <button
                (click)="openModal()"
                class="du-btn du-btn-sm du-btn-outline du-btn-accent w-full gap-2"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-4 w-4"
            ...
            >
                @for (task of list().tasks; track task.id) {
                    ...
                    <div class="py-6 text-center">
                        ...
                        <p class="text-base-content/60 mt-2 text-sm italic">
                            No tasks in {{ list().title }}
                        </p>
                    </div>
                }
            </div>
        ...
        <app-modal-component [modalId]="editTitleModalId()">
            <app-form-edit-list
                [list]="list()"
                (listUpdated)="handleEditForm($event)"
            ></app-form-edit-list>
        </app-modal-component>
    `,
    styles: ``,
...
export class ListComponent {
    ...
    ngOnInit() {
        this.connectedDropLists =
            this.boardService
                .boards()
                .find((b) => b.id === this.boardId())
                ?.lists.map((l) => this.getDropListId(l.id)) ?? [];
    }
    ...
    addTask(title: string, description: string, tag: string) {
        ...
        this.boardService.addTaskToList(
            this.boardId(),
            this.list().id,
            newTask,
        );
    ...
    editTitleModalId = computed(
        () => `modal-edit-title-list-${this.list().id}`,
    );
    ...
    handleEditForm(updatedList: ListModel) {
        this.boardService.updateListTitle(
            this.boardId(),
            updatedList.id,
            updatedList.title,
        );
