    ...
    template: `
        <div class="group" cdkDrag [cdkDragData]="task">
            ...
            >
                <pre
                    data-prefix="ttl"
                ><code class="font-bold text-sm">{{ task().title }}</code></pre>
                <pre
                    data-prefix="dsc"
                ><code class="text-sm opacity-90">{{ task().description }}</code></pre>
                <pre
                    data-prefix="tag"
                ><code class="text-xs"><span class="du-badge du-badge-accent du-badge-sm">{{ task().tag }}</span></code></pre>
            </div>
        </div>
        ...
        <app-modal-component (openModal)="openModal()" [modalId]="modalId()">
            <div class="flex flex-col gap-4">
                ...
                <button
                    (click)="requestDelete()"
                    class="du-btn du-btn-outline du-btn-error gap-2"
                >
                    <svg
        ...
        >
            <app-edit-task-form
                (taskUpdated)="handleEditForm($event)"
                [task]="task()"
            ></app-edit-task-form>
        </app-modal-component>
    `,
